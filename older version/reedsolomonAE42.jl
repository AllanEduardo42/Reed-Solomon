...
#    using the provided primitive polynomial.'''
    ...
    global gf_exp = zeros(UInt8,510)
    global gf_log = zeros(UInt8,255)
...
function gf_mult(x,y)
    ...
    return gf_exp[0 + gf_log[x] + gf_log[y]]
end
...
function gf_div(x,y)
    ...
    return gf_exp[(255 + gf_log[x]) - gf_log[y]]
end
...
function gf_pow(x,power)
    if power == 0
        ...
        return gf_exp[255 + mod(power*gf_log[x], 255)]
    end
end
