...
function gf_mult_noLUT(p, q)
    ...
    if q == gf_prim # just shift the polynomial
        if init_tables_flag == true
            result = [gf_convert(p); 0]
        else
            result = [p; 0]
        end
    else
    ...
    if length(result) >= length(prim_poly_gf)
        ___, result = gf_div_noLUT(result, prim_poly_gf)
        return result
    elseif init_tables_flag == true
        return anti_gf_convert(result)
    else
        return result
    end
...
function gf_div_noLUT(dividend, divisor)
    ...
    if init_tables_flag == true
        return anti_gf_convert(dividend[1:separator]), anti_gf_convert(dividend[separator+1:end]) # return quotient, remainder.
    else
...
function gf_convert(x)
    ...
    if x > field_charac
        throw(ArgumentError("x is larger than "*string(field_charac)*"."))
    end
    quocient = x ÷ gf_prim
    remainer = rem(x, gf_prim)
    coeffs = [remainer]
    i = 1
    while quocient >= gf_prim
        remainer = rem(quocient, gf_prim)
        quocient = quocient ÷ gf_prim
...
function init_tables(generator)

    global gf_exp = zeros(Int, field_charac * 2) # anti-log (exponential) table
...
function gf_oposite(p)
    ...
    r = [mod_oposite[p[i]+1] for i in eachindex(p)]
    if init_tables_flag == true
        r = anti_gf_convert(r)
    end
    return r
...
function gf_div(x,y)
    ...
    if init_tables_flag == true
        return gf_exp[field_charac + gf_log[x] - gf_log[y]]
    else
...
function gf_pow(x,power)
    ...
    if init_tables_flag == true
        return gf_exp[field_charac + rem(power*gf_log[x], field_charac)]
    else
        y = 1
        for i=1:power
            y = gf_mult(y,x)
...
function gf_inverse(x)
    ...
    if init_tables_flag == true
        diff = field_charac - gf_log[x]
        if diff == 0
            return gf_exp[field_charac]
        else
            return gf_exp[field_charac - gf_log[x]]
        end
    else
        return gf_mult_noLUT([1], x)
    end
end
...
# '''Multiply two polynomials, inside Galois Field.'''
    # Pre-allocate the result array
    r = zeros(Int,length(p) + length(q) - 1)
    # compute the polynomial multiplication (just like the outer product of two
    # vectors, we multiply each coefficent of p with all coefficients of q)
    for j in eachindex(q)
        for i in eachindex(p)
...
function rs_encode_msg(msg_in, nsym; generator=GEN, prim=PRIM, prim_poly=PRIM_POLY, c_exp=C_EXP)
    ...
    global gf_prim = prim
    global field_charac = Int(prim^c_exp - 1)
    init_oposites()
    init_inverses()
    if field_charac < 100000
        # convert the primitive polinomial
        global init_tables_flag = true
        if length(prim_poly) == 1
            quocient = prim_poly ÷ prim
            remainer = rem(prim_poly, prim)
            global prim_poly_gf = [gf_convert(quocient); remainer] # since prim_poly
                                                # outside the GF(prim^c_exp), we calculate
                                                # the first remainer before convertion.
        else
            global prim_poly_gf = prim_poly
        end
        init_tables(generator)
    else
        global init_tables_flag = false
        if length(prim_poly) == 1
            quocient = Int(prim_poly ÷ prim)
            remainer = Int(rem(prim_poly, prim))
            global prim_poly_gf = [gf_convert(quocient); remainer] # since prim_poly
                                                # outside the GF(prim^c_exp), we calculate
                                                # the first remainer before convertion.
        else
...
function rs_calc_syndromes(msg, nsym; generator=GEN)
...
#    equivalent to a Fourier Transform (Chein search being the inverse).'''
    ...
    for i in eachindex(synd)
        synd[i] = gf_poly_eval(msg, gf_pow(generator,i-1))
    end
    return [0; synd]
end
    ...
    for i = 0:(nsym-erase_count-1) # generally: nsym-erase_count == length(synd),
        ...
        if delta != 0 # Update only if there's a discrepancy
            ...
            #                        # as long as L is correctly computed.
                ...
                println("delta=",anti_gf_convert(delta))
                println("inverse_delta=",anti_gf_convert(gf_inverse(delta)))
...
function rs_find_errors(err_loc, nmess, generator) # nmess is length(msg_in)
...
#   constant time).'''
    ...
    for i=0:nmess-1 # normally we should try all 2^8 possible values, but
        ...
        if gf_poly_eval(err_loc, gf_pow(generator,i)) == 0 # it's a 0? Bingo, it's a root
...
function rs_correct_errata(msg_in, synd, err_pos, generator)
...
#   message.'''
    ...
    coef_pos = [length(msg_in) - p for p in err_pos] # need to gf_convert the 
...
function rs_find_errata_locator(e_pos, generator)
...
#    are n-1 - [1, 4] = [18, 15] = erasures_loc to be specified as an argument.'''
    ...
    for i in e_pos
        e_loc = gf_poly_mult(e_loc, gf_poly_sub([1], [gf_pow(generator, i); 0]))
    end
    return e_loc
end
...
#'''Reed-Solomon main decoding function'''
    ...
    synd = rs_calc_syndromes(msg_out, nsym; generator)
    ...
    println("err_loc=",err_loc)
    # locate the message errors using Chien search (or brute-force search)
    err_pos = rs_find_errors(err_loc[end:-1:1], length(msg_out), generator)
    ...
    # and erasures.
    msg_out = rs_correct_errata(msg_out, synd, [erase_pos; err_pos], generator) # note that
...
# msg_encoded = rs_encode_msg(msg, nsym; prim_poly=4294967437,c_exp=32)
