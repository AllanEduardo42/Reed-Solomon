...
function gf_inverse(x)
    q, r = gf_div_noLUT([zeros(Int,length(x)); 1], x)
    return q
end
    ...
    for i = 0:(nsym-erase_count-1) # generally: nsym-erase_count == length(synd),
        ...
        if delta != 0 # Update only if there's a discrepancy
            ...
            #                        # as long as L is correctly computed.
                ...
                println("old_loc1=",[anti_gf_convert(old_loc[i]) for i=1:length(old_loc)])
                println("inverse=", anti_gf_convert(gf_inverse(delta)))
                old_loc = gf_poly_scale(err_loc, gf_inverse(delta))
                println("old_loc2=",[anti_gf_convert(old_loc[i]) for i=1:length(old_loc)])
...
#   constant time).'''
    ...
    for i=0:nmess-1 # normally we should try all 2^8 possible values, but
        ...
        if anti_gf_convert(gf_poly_eval(err_loc, gf_pow(generator,i))) == 0 # it's a 0? Bingo, it's a root
...
#'''Reed-Solomon main decoding function'''
    ...
    println("err_loc=",[anti_gf_convert(err_loc[i]) for i=1:length(err_loc)])
...
msg_encoded = rs_encode_msg(msg, nsym; prim_poly=poly,c_exp=8)
